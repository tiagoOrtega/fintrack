 ...
 *      and http://localhost:5173/settings to "Authorized redirect URIs"
 */
...
export async function handleGoogleCallback(code: string, state: string): Promise<GoogleTokens> {
  ...
  // Extract email from the id_token JWT payload (no signature verification needed for own token)
  let userEmail = ''
